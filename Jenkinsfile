    ...
    stages {
        ...
        stage('Build Frontend Image') {
            steps {
                script {
                    dir('frontend') {
                        FRONT_DOCKER = docker.build("${FRONT_IMAGE}:${env.FRONT_TAG}", ".")
                    }
                }
            }
        }
        ...
        stage('Build Backend Image') {
            steps {
                script {
                    dir('backend') {
                        BACK_DOCKER = docker.build("${BACK_IMAGE}:${env.BACK_TAG}", ".")
                    }
                }
            }
        }
                ...
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'hyomee2') {
                        ...
                        if (env.FRONT_CHANGED == "true") {
                            FRONT_DOCKER.push("${env.FRONT_TAG}")
            ...
            steps {
                /* GKE 인증 */
                 ...
                 script {
                    ...
                    sh """
                        kubectl set image deployment/${backTargetDeploy} backend=${BACK_IMAGE}:${env.BACK_TAG} -n ${NAMESPACE}
                 ...
                 script {
                    if (env.FRONT_CHANGED == "true") {
                        ...
                        sh """
                            kubectl set image deployment/${frontTargetDeploy} frontend=${FRONT_IMAGE}:${env.FRONT_TAG} -n ${NAMESPACE}
